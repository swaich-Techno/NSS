...
function safeColor(value: string | null | undefined, fallback = "#145f45") {
...
function infoCard({
  ...
  const content = [rect(x, y, width, height, { fill: "#ffffff", stroke: "#d7ded6", lineWidth: 0.8 })];
  content.push(text(title, x + 14, y + height - 22, { size: 8, font: "F2", color: "#65715f" }));
  let cursor = y + height - 43;
  for (const row of rows) {
    content.push(text(row.label, x + 14, cursor, { size: 7, font: "F2", color: "#7a8476" }));
...
export function createInvoicePdf({
  ...
  content.push(rect(0, 0, 612, 792, { fill: "#fbfcf8" }));
  content.push(rect(0, 700, 612, 92, { fill: primary }));
  content.push(rect(0, 696, 612, 4, { fill: accent }));
  content.push(rect(left, 728, 48, 48, { fill: "#ffffff", stroke: "#ffffff", lineWidth: 1 }));
  ...
  const headerLines = [
    ...
    [settings.gstin ? `GSTIN: ${settings.gstin}` : null, settings.fssaiLicense ? `FSSAI: ${settings.fssaiLicense}` : null].filter(Boolean).join(" | ")
  ]
    .filter(Boolean)
  ...
  content.push(text("INVOICE", 424, 758, { size: 24, font: "F2", color: "#ffffff", align: "right", width: 146 }));
  ...
  const paymentRows = [
    ...
    { label: "Cashier", value: cashierName || "System" }
  ];
  content.push(...infoCard({ title: "PAYMENT", rows: paymentRows, x: 318, y: 584, width: 252, height: 92 }));

  const tableTop = 536;
  content.push(rect(left, tableTop, width, 28, { fill: "#eef5ed", stroke: "#d7ded6", lineWidth: 0.8 }));
  const columns = [
  ...
  for (const column of columns) {
    content.push(text(column.label, column.x, tableTop + 10, { size: 8, font: "F2", color: "#475246" }));
  }
  ...
  if (displayedItems.length === 0) {
    ...
    displayedItems.forEach((item, index) => {
      ...
      content.push(rect(left, rowY, width, rowHeight, { fill: index % 2 === 0 ? "#ffffff" : "#f7faf4", stroke: "#e1e7df", lineWidth: 0.5 }));
  ...
  content.push(line(left, rowY + 1, right, rowY + 1, "#d7ded6", 0.8));
  const summaryTop = Math.min(rowY - 20, 298);
  const notesTop = summaryTop;
  const notesBottom = 86;
  content.push(rect(left, notesBottom, 280, notesTop - notesBottom, { fill: "#ffffff", stroke: "#d7ded6", lineWidth: 0.8 }));
  ...
  totals.forEach(([label, value]) => {
    content.push(rect(354, totalY - 23, 216, 23, { fill: "#ffffff", stroke: "#e1e7df", lineWidth: 0.5 }));
