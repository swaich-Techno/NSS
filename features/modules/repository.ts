...
function filterDemoRows(rows: ModuleRow[], search?: string) {
...
export async function listModuleRecords(key: string, options: ListOptions = {}): Promise<ModuleListResult> {
  ...
  const fallbackRows = filterDemoRows(definition?.demoRows ?? [], options.search);
  const fallback = {
...
async function countModuleRows(client: PrismaClient, key: string, search?: string) {
  switch (key) {
    ...
    case "inventory":
      return client.inventoryItem.count({ where: textSearch(search, ["name", "batchNumber"]) as never });
    case "farmers":
      return client.milkCollection.count({
        where: search ? { farmer: { name: { contains: search, mode: "insensitive" } } } : undefined
      });
    case "suppliers":
      return client.supplier.count({ where: textSearch(search, ["name", "phone", "gstOrLicense"]) as never });
    case "employees":
    ...
    default:
      return getModuleDefinition(key)?.demoRows.length ?? 0;
  }
}
  ...
  switch (key) {
    ...
    case "inventory": {
      const items = await client.inventoryItem.findMany({
        where: textSearch(search, ["name", "batchNumber"]) as never,
        orderBy: { name: "asc" },
        skip,
        take
      });
    ...
    case "suppliers": {
      const suppliers = await client.supplier.findMany({
        where: textSearch(search, ["name", "phone", "gstOrLicense"]) as never,
        orderBy: { name: "asc" },
        skip,
        take
      });
      return suppliers.map((supplier) => ({
        ...
        payable: decimal(supplier.openingBalance),
    ...
    default:
      return filterDemoRows(getModuleDefinition(key)?.demoRows ?? [], search).slice(skip, skip + take);
  }
}
