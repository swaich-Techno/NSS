...
export function demoRoleForEmail(email: string): UserRoleKey {
  if (email.startsWith("agency")) return "SUPER_ADMIN";
  if (email.startsWith("manager")) return "MANAGER";
  if (email.startsWith("cashier")) return "CASHIER";
  if (email.startsWith("inventory")) return "INVENTORY_STAFF";
  if (email.startsWith("accountant")) return "ACCOUNTANT";
  if (email.startsWith("viewer")) return "READ_ONLY_STAFF";
  return "OWNER";
}
