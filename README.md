...
If `DATABASE_URL` is not configured, the app still allows demo login and renders a zero-data fallback UI so the interface can be reviewed before database setup.
...
- Run `npm run db:seed` to create roles, demo login users, and base business settings. It intentionally does not create business transactions or fake records.
- Run `npm run db:reset-business-data` only when you intentionally want to clear products, invoices, ledgers, inventory, customers, farmers, suppliers, employees, expenses, payments, reports, and audit logs while keeping users, roles, permissions, and business settings.
